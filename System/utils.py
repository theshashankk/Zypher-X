...
from System import LOAD_PLUG
from System import CMD_LIST
import re
import logging
import inspect
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        sys.modules["System.utils"] = System.utils
        sys.modules["userbot"] = System
...
def load_pmbot(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"plugins/mybot/pmbot/{shortname}.py")
        name = "plugins.mybot.pmbot.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["plugins.mybot.pmbot." + shortname] = mod
